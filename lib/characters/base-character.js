import { newArray, isEmpty, contains } from '../utils/utils';
...
export default class BaseCharacter {
  ...
  animate(type, directions, interrupts, spriteDirection) {
    ...
    var isAttacking = contains(interrupts, 'melee');

    console.log(directions, interrupts);

    if (isEmpty(directions)) {
      this.stop();
      this.clearAnimation();
    } else if (interrupts.length > 0 && !this.isInterrupted) {
      this.clearAnimation();
      this.movement.setDirection(directions);
      this.isInterrupted = true;
      this.setAnimation.call(this, directions, isAttacking);
  ...
  setAnimation(directions, isAttacking) {
    this.animationIndex = 0;
    isAttacking ? this.attack() : this.move();
